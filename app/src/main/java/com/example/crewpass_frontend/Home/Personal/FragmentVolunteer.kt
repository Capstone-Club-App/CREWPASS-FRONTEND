...
class FragmentVolunteer : Fragment() {
    ...
    fun initRecyclerView(){
        recent_list.apply {
            add(Announcement("봉사 최신 동아리1", "제목1", "내용1"))
            add(Announcement("봉사 최신 동아리2", "제목2", "내용2"))

            homeRecentRVAdapter = HomeRecentRVAdapter(recent_list)
            binding.volunteerRecentRv.adapter = homeRecentRVAdapter
            binding.volunteerRecentRv.layoutManager = LinearLayoutManager(context)
        ...
        imminent_list.apply {
            add(Announcement("봉사 마감임박 동아리1", "제목1", "내용1"))
            add(Announcement("봉사 마감임박 동아리2", "제목2", "내용2"))

            homeImminentRVAdapter = HomeImminentRVAdapter(recent_list)
            binding.volunteerImminentRv.adapter = homeImminentRVAdapter
            binding.volunteerImminentRv.layoutManager = LinearLayoutManager(context)
